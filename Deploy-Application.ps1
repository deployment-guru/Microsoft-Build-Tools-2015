...
Try {
    ...
    [String]$appName = 'Microsoft Build Tools 2015'
    [String]$appVersion = ''
    [String]$appArch = ''
    [String]$appLang = 'DE'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		$32BitFilePath = Get-ChildItem -Path "$dirFiles" -Include BuildTools_Full.exe -File -Recurse -ErrorAction SilentlyContinue
        If($32BitFilePath.Exists) {
			Execute-Process -Path "$32BitFilePath" -Parameters "/quiet /norestart /log $configToolkitLogDir\MSBuildTools2015_Install.log" -WindowStyle Hidden  
        }
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
		...
		$SourceCache = Get-ChildItem -Path "$dirFiles" -Include BuildTools_Full.exe -File -Recurse -ErrorAction SilentlyContinue
        If($SourceCache.Exists) {
			Execute-Process -Path "$SourceCache" -Parameters "/uninstall /quiet /norestart /log $configToolkitLogDir\MSBuildTools2015_Uninstall.log" -WindowStyle Hidden  
        }
